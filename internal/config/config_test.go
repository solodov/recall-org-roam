...
func TestLoadBytesDefaultsIndexDirectoryFromXDGDataHome(t *testing.T) {
	...
	if got, want := cfg.IndexDirectory, filepath.Join(xdgDataHome, "org-search", defaultIndexDirectoryName); got != want {
...
func TestLoadBytesFallsBackToLocalShareDefaultIndexDirectory(t *testing.T) {
	...
	if got, want := cfg.IndexDirectory, filepath.Join(homeDir, ".local", "share", "org-search", defaultIndexDirectoryName); got != want {
		t.Fatalf("index_directory = %q, want %q", got, want)
	}
}
